...
log = Log.open_topic ("audio-links")

local function log_info  (msg) log:info    ("[audio-links] " .. msg) end
local function log_warn  (msg) log:warning ("[audio-links] " .. msg) end
local function log_debug (msg) log:debug   ("[audio-links] " .. msg) end
...
local function find_port (source, node_name, port_name)
  ...
  for port in node:iterate_ports {
    ...
    log_debug ("find_port: found " .. node_name .. ":" .. port_name
      .. " object.id=" .. (port.properties ["object.id"] or "?"))
    return port
  end
...
local function try_create_links (source)
  ...
  for _, spec in ipairs (LINKS) do
    ...
    if active_links [key] then
      ...
      log_info (string.format ("  output %q -> %s", spec.output,
        out_p and ("id=" .. (out_p.properties ["object.id"] or "?")) or "NOT FOUND"))
      log_info (string.format ("  input  %q -> %s", spec.input,
      ...
      if out_p and in_p then
        ...
        link:activate (Feature.Proxy.BOUND, function (l, e)
          if e then
            ...
            log_info ("link active: " .. key
              .. " bound-id=" .. tostring (l ["bound-id"]))
            active_links [key] = l
          end
        end)
      else
